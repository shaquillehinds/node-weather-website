...
const forecast = (lat, long, callback) => {
  const url = `http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=${process.env.API_KEY}`;
  request({ url, json: true }, (err, { body }) => {
    if (err) {
      callback("Unable to connect to weather service.", undefined);
    } else if (!(body.cod === 200)) {
      callback(`Unable to gather weather info for those coordinates. Error Code: ${body.cod}`);
    } else {
